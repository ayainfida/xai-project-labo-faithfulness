...
def linear_probe_sklearn_main(cfg):
    ...
    os.makedirs("output/linear_probe", exist_ok=True)
...
def asso_opt_main(cfg):
    ...
    required_concept_files = [cfg.cls_name_path, cfg.raw_sen_path, cfg.concept2cls_path]
    if any(not os.path.isfile(path) for path in required_concept_files):
        print("Missing concept cache files. Regenerating from concept JSON...")
        concept_json = cfg.concept_root + cfg.concept_json if 'concept_json' in cfg else cfg.concept_root + "class2concepts.json"
        class2concepts = json.load(open(concept_json, "r"))
        # Optional name mapping: cfg.concept_name_map = {"planes": "airplane", ...}
        if 'concept_name_map' in cfg and cfg.concept_name_map:
            class2concepts = {cfg.concept_name_map.get(k, k): v for k, v in class2concepts.items()}
    ...
    if cfg.test:
        ...
        model = AssoConceptFast.load_from_checkpoint(str(ckpt_path), weights_only=False)
        trainer = pl.Trainer(devices=1, accelerator="cuda")
    ...
    if not cfg.DEBUG:
        ...
        # early stopping to prevent overfitting, especially for small shot setting and to not waste too much time for training
        early_stop_callback = pl.callbacks.EarlyStopping(
            monitor='val_acc',
            mode='max',
            patience=10,          # 8 validation checks
            min_delta=1e-3,
            verbose=True
        )
        ...
        trainer = pl.Trainer(devices=1, accelerator="cuda", max_epochs=1000, \
